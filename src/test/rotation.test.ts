...
test("fixed rotation cycles through the declared slots", () => {
  const r = simulateScenario(scenario({ turns: 4, rotation: ["active1", "basic"] }), customRegistry({}));
...
test("ultimate is only used when Confectance covers its cost (rotation scans ahead)", () => {
  ...
  const r = simulateScenario(scenario({ turns: 5, rotation: ["ultimate", "basic"], keys: [] }), customRegistry({}));
  assert.deepEqual(actionsOf(r), [
...
test("falls back to basic when every rotation slot is unavailable", () => {
  ...
  const r = simulateScenario(scenario({ turns: 3, rotation: ["ultimate"], keys: [], config: { confectanceStart: 0 } }), customRegistry({}));
...
test("cooldown model (U11 CONFIRMED): CD-1 cast Turn N → unavailable N+1 → available N+2", () => {
  ...
  const r = simulateScenario(scenario({ turns: 3, rotation: ["active1"] }), customRegistry({}));
