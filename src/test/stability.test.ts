...
test("stability damage is per-hit fixed value; no break when stability > 0", () => {
  ...
  const r = simulateScenario(scenario({ turns: 1, dummy: { stability: 9 } }), customRegistry({}));
  const ev = r.log[0];
...
test("break at stability 0 sets the exposed flag and respects configured duration", () => {
  ...
  const r = simulateScenario(scenario({ turns: 2, dummy: { stability: 4 }, config: { exposedDurationRounds: 2 } }), customRegistry({}));
...
test("a dummy with stability 0 never breaks (no invented collapsed-at-start semantics)", () => {
  const r = simulateScenario(scenario({ turns: 5 }), customRegistry({}));
...
test("warnings mention the broken-window knob only when it is overridden", () => {
  const dflt = simulateScenario(scenario({ turns: 3, dummy: { stability: 5 } }), customRegistry({}));
  const overridden = simulateScenario(scenario({ turns: 3, dummy: { stability: 5 }, config: { exposedDurationRounds: 3 } }), customRegistry({}));
...
test("No-Cover: Stability > 0 never reduces damage (U5 boss-domain — pure resource)", () => {
  ...
  const noStab = simulateScenario(scenario({ turns: 5, seed: 5, rotation: ["basic"] }), customRegistry({}));
  const stab = simulateScenario(scenario({ turns: 5, seed: 5, rotation: ["basic"], dummy: { stability: 9 } }), customRegistry({}));
  assert.deepEqual(
...
test("U3 resolved: no universal Exposed damage multiplier — Broken targets take normal damage", () => {
  ...
  const broken = simulateScenario(scenario({ turns: 5, seed: 5, rotation: ["basic"], dummy: { stability: 2 } }), customRegistry({}));
  const intact = simulateScenario(scenario({ turns: 5, seed: 5, rotation: ["basic"], dummy: { stability: 9 } }), customRegistry({}));
  const zeroFromStart = simulateScenario(scenario({ turns: 5, seed: 5, rotation: ["basic"], dummy: { stability: 0 } }), customRegistry({}));
