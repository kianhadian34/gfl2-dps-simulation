...
test("each damage event gains exactly +1 Confectance", () => {
  const r = simulateScenario(scenario({ turns: 3, rotation: ["basic"], keys: [] }), customRegistry({}));
...
test("Ultimate costs exactly 3 Confectance, consumed immediately on activation", () => {
  const r = simulateScenario(scenario({ turns: 1, rotation: ["ultimate"], keys: [] }), customRegistry({}));
