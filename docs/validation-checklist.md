...
| 30 | Fixed Damage (U21) — **post-chain, independent ceil**: `ceil(normalChain) + ceil(fixed)`; bypasses DEF / damage-buff bracket / phase / weakness / reduction / crit | **CONFIRMED (in-game: Overburn 196 = ceil(10% × 1958 ATK), immune to Burn weakness and +20% No-Cover buff; 2026: bypassed ordinary 80% Damage Reduction — Overburn 1949 → 195, unchanged)**; DEF/crit/phase bypass and **Final DMG Reduction vs fixed** SOURCE-SUPPORTED (untested in-game) | absolute `SkillDef.fixedDamage` (percentOfAtk data model deferred) | `damage.test.ts`, `fixed-damage-validation.test.ts`; Final DMG Reduction = test item (row 36) |
...
| 36 | **Fixed Damage vs FINAL DMG Reduction — TEST ITEM (not an implementation task):** the source formula places Final DMG Reduction in the fixed-damage calculation (fixed DMG = … × final DMG reduction, rounded up); ordinary Damage Reduction was validated 2026 as BYPASSED by fixed damage (Overburn 1949 → 195 vs 80% DR), but Final DMG Reduction is a SEPARATE mechanic and this does NOT imply fixed bypasses it. Requires its own in-game validation (e.g., an enemy/effect explicitly granting Final DMG Reduction). The boss −80% passive's classification (ordinary vs Final DMG Reduction) also remains to be determined | UNVALIDATED (source formula only) | — (no engine change until validated) | — (in-game test pending) |
