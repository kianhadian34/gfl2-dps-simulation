...
| 34 | Weakness-matching counts (U15a, 2026) — weakness factor = ADDITIVE matched count `1 + 0.10 × n`; element AND Ammo-tag matches count into the SAME factor; 1 matched → ×1.10 (Burn-only 1091, repeated 4×), 2 matched → ×1.20 (Burn+Ammo 1191 normal / 1470 crit @123.5% CDMG); AWU separate (Physical-only). NOT claimed: zero-weakness Phase target and Ammo-only Phase target (dummy tool limitations); partial-match edge untested | CONFIRMED (matched counts, in-game 2026) — partial-match edge UNTESTED | — (existing generic weakness path) | `weakness-matching-validation.test.ts` (1-weakness, 2-weakness, crit, factor independence) |
...
- `npm test` → 122/122 pass (92 base + 7 U5 boss-Stability + 2 U7/U8 status-timing + 2 U14 boss-DEF + 10 Ammo Weakness Upgrade + 4 Phase-elemental-weakness U15b + 4 weakness-matching U15a + 1 AWU persistence).
