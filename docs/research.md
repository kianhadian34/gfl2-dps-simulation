...
**Implementation interpretation** — slot model `AbilityDef` per ability (`basic, active1, active2, ultimate, support, passive`), each with **level-indexed complete variants** (`levels: { level → SkillDefVariant }`, where a `SkillDefVariant` is the full behavior definition: multiplier|fixedDamage, element, ammo, stab, cooldown, cost, statuses, at-max hook). **Skill levels & Fortifications (architected 2026):** every ability starts at Level 1; **Basic Attack is always Level 1** (never raised by Fortification); each Fortification raises ONE specific ability to an explicit `toLevel` (`CharacterDef.fortificationMap: { v, ability, toLevel }[]`); a higher level is a COMPLETE variant and may change math, hits, effects, durations, resources, cooldowns, Stability, targeting/AoE, or triggers. Resolution (`state.ts`): level = toLevel of the entry with highest `v ≤ run fortificationLevel` (never inferred by counting); an explicitly requested level with no variant FAILS clearly; **baseline rule**: an un-upgraded ability with only a higher level in data resolves to its lowest available variant. **QJ state (authoritative kit sync 2026):** Basic Lv1 only (80%/physical/AR/stab2 — "Ammo Type: Medium" in the UI; repo identifier `assault_rifle_ammo` — equivalence requires confirmation, NOT silently renamed); **Common Rail Lv1 = 150% ATK / Burn / stab 3 / cd1 / self Support Boost I** (the earlier 'Lv2 = 150%' reading was a misplacement — corrected), Lv2 (V1) adds the kill-condition Support-Boost-I change (DEFERRED); **Guide to Victory Lv1 = 110% / Burn / stab 3** (corrected from 0), Lv2 (V2) +100% crit rate vs Overburn-inflicted targets (DEFERRED); **Pressing the Momentum** Lv1 executed (cost 3, SB II ×3, at-max +1 stack & +1 Support Action), Lv2 (V4) Vulnerable on No-Cover targets (DEFERRED — Cover-gated application unsupported), Lv3 (V5) Damage Up II to self+ally before the allied hit (DEFERRED — pre-ally timing unsupported); **Steady Plan is level-aware** (Lv1 executed; Lv2 (V3) Overburn-after-Support + Support-damage +10% DEFERRED; Lv3 (V6) adds a SECOND +10% No-Cover component, EXECUTED as a separate additive component). **Evidence per the project source hierarchy (authoritative, `validation-checklist.md` §0/§8):** the Lv1 +10% and the additional V6 +10% No-Cover bonuses are **source facts** (tooltip-stated; no lower-Fortification gameplay test is required to establish them); the observed V6 brackets reproduce **1.20** (1091/1191/992/1207/2233/2340); the additive combination `1 + 0.10 + 0.10 = 1.20` is therefore **Mathematically Proven** (multiplicative 1.10×1.10 = 1.21 is inconsistent with the observed results). **V6 is the authoritative experimental state for Qiongjiu because it is the available character state.** **fortificationMap populated**: V1→Common Rail Lv2, V2→Guide Lv2, V3→Steady Plan Lv2, V4→Ult Lv2, V5→Ult Lv3, V6→Steady Plan Lv3.
...
**Mechanic** — Four key tables: Fixed (专属, doll-specific, several equippable; Qiongjiu's six are recorded in `src/data/qiongjiu.ts` — FK1 Concentration +3 Confectance at battle start [IMPLEMENTED]; FK2 Efficient Planning [cleanses 1 buff before Support Action — deferred: no buff-purge mechanic in MVP]; FK3 Targeted Training [Defense Down II 1 turn before the allied unit's attack while in Support Mode — deferred: support fires AFTER the triggering hit, so pre-attack application cannot be faithfully ordered]; FK4 Point of Vulnerability [Guide to Victory AoE 8 tiles, −30% to non-first targets — deferred: multi-target out of MVP scope]; FK5 Necessary Adjustments [Blazing Assault II 2 turns on phase-weakness exploit via Common Rail — deferred: needs blazing_assault_ii definition + skill-scoped trigger]; FK6 Steadiness [displacement immunity under Support Boost — deferred: no displacement mechanics in MVP]), Common (shared pool, stat + conditional effect, e.g. crit+5% / +7% dmg on others' turns — NOT in current scope), Expansion (1 per doll, playstyle-changing: Qiongjiu's Ruined Gem — support damage becomes Burn, +15% vs Burn-debuff targets — **recorded, deferred**: needs skill-element mutation and a 'target has Burn debuff' condition, neither representable today), Affinity (bond-5: Qiongjiu's Warm as Jade — at Affinity 5: CD+3.3%/ATK+3.3%/HP+3.3%; at Affinity 9: CD+4.5%/ATK+4.5%/HP+4.5% [the earlier 5% Lv9 value was incorrect — confirmed 4.5%]; 9 levels total; Levels 6–8 NOT assumed/interpolated; applies only when equipping her OWN Affinity Key — recorded, engine consumption deferred: Crit Damage is not a `stat_modifier` stat and affinity modifies the panel).
