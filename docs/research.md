...
**Implementation interpretation** — **IMPLEMENTED (2026, Golden Melody C1-scope, fully data-driven):** `WeaponDef` gains `calibrations` (per-calibration Effect data, C1–C6 from the documented table) + `calibrationLevel`; the resolved calibration's **Damage Dealt** enters the existing additive DMG% bucket for every attack (975 validated) and its **Charging** counter contributes per-stack Support Action damage (support-scoped, same bucket — 1434 validated). **Charging is a weapon-effect CHARGE COUNTER** (not a status): +1 when the holder GAINS A BUFF (new application, capped by the calibration's maxStacks — validated trigger), persists when unused, 1 consumed per Support Action, inherently un-cleansable. `calibrationLevel` is **ABSENT by default** so the established pre-weapon validations (all observed WITHOUT the calibration Effect) are preserved byte-for-byte; setting a level activates that calibration's Effect generically (no per-calibration branches; `weapon` provenance kind). **Trait and Imprint remain NOT implemented** (Trait pool UNKNOWN; Imprint combat untested); "Activations" is not modeled (unvalidated meaning). No weapon registry / `weaponId` / trait / imprint systems yet.
