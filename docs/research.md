...
**Remaining uncertainty (U15a core edge — UNTESTED):** partial-match semantics — a target exposing weaknesses the attack does NOT match (e.g. weaknesses Burn+Decay vs a Burn attack) — remains unvalidated; the engine implements it by counting only matched weaknesses (inference, not observation).
...
**Remaining unknowns (NOT resolved, deliberately):** whether any EXPLICIT mechanic removes or resets AWU stacks (e.g. Stability break/recovery or a boss rotation) — **unobserved, not invented** (time-based expiry is none, validated 2026); whether exploiting an Ammo weakness also grants the generic +2 stability weakness bonus (the engine keeps the +2 stability bonus element-scoped, Stability behavior unchanged).
