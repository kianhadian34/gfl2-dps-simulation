...
- **Weapon mechanics are NOT assumed identical to Key mechanics** — Effect / Trait / Imprint are separate concepts; the **Trait** and **Imprint** systems and **Activations** behavior are NOT implemented (see evidence status below).
...
These are **13 DIRECTLY OBSERVED outcomes — NOT necessarily the complete random pool**. When the valid Trait trigger occurs, Golden Melody selects **ONE buff** from the current observed pool of 13. The simulator does **NOT** currently implement this selection; **Trait RNG is deferred**. Do NOT invent probability weights or distributions; do NOT state or imply that the 13 outcomes are necessarily equally weighted; do NOT invent duplicate behavior or other unsupported Trait mechanics. Whether the complete pool and all outcomes share identical internal timing/implementation remain **UNKNOWN** — do NOT invent additional outcomes; do NOT assume all Trait buffs use the same engine mechanic; do NOT infer timing/implementation beyond what was directly observed. Their displayed effects are recorded verbatim above. When implemented, the existing deterministic seeded RNG should make the selection reproducible.
