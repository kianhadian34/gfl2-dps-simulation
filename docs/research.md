...
**Unknowns** — partial-match (U15a: exploiting only some of a target's weaknesses) is implied by the count-based rule and implemented by counting matched weaknesses, but not separately in-game-tested; phase×weakness for ELEMENT weakness matches is RESOLVED (U15b, 2026 — the generic ×1.10 factor applies to Phase damage); the phase-WHEEL (counter-relation) interaction remains UNKNOWN until the wheel is populated.
...
| U15 | Weakness partial-match (U15a) & phase×weakness interaction (U15b) — **U15b RESOLVED 2026**: the generic elemental weakness multiplier applies to Phase damage exactly as to Physical (1 matched element weakness → ×1.10; validated: Burn Common Rail 2233 no-weakness baseline → 2340 with Burn weakness); separate from AWU (§3.18), no new Phase-specific mechanic; the phase-WHEEL (counter-relation) half of the interaction still depends on the unpopulated wheel. **U15a (partial-match) remains UNRESOLVED** | U15b **CONFIRMED (in-game)** / U15a UNKNOWN | Edge-case damage | U15b: `phase-weakness-validation.test.ts` ✅; U15a: weakness test |
