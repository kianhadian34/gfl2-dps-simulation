...
| `stability` | Stability damage, break/expose, recovery (**confirmed 2-turn delay after break → restore to max**, research U6). No-Cover boss domain: no universal stability damage reduction (generic rule is Cover-scoped); boss-specific stability-conditional passives = in-scope design note (research §3.7), not yet built (no boss data) |
